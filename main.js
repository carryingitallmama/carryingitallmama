...
const RSS_PROXIES = [
  feed => `https://rss-proxy.steep-truth-006b.workers.dev/`,
  feed => `https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(feed)}&count=20`,
...
async function fetchEpisodes(rssUrl) {
  ...
  // Try Cloudflare Worker first — returns raw RSS XML directly
  try {
  ...
  // Try corsproxy.io as final fallback — returns raw XML
  try {
    const res = await fetch(RSS_PROXIES[2](rssUrl));
